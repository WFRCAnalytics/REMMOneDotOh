...
def parcel_volume(parcels, year, settings):
    if year < 2020:
        file = settings['tdm']['main_dir'] + '2015/6_REMM/Ro/volumepoint.dbf'
        ...
        file = settings['tdm']['main_dir'] + '2019/6_REMM/Ro/volumepoint.dbf'
        s = wfrc_utils.dbf2df(file).set_index('parcel_id')
        svolume = s.RASTERVALU.reindex(parcels.index).fillna(0)
    elif year in range(2028,2036):
        file = settings['tdm']['main_dir'] + '2027/6_REMM/Ro/volumepoint.dbf'
        s = wfrc_utils.dbf2df(file).set_index('parcel_id')
        svolume = s.RASTERVALU.reindex(parcels.index).fillna(0)
    ...
    #    p = wfrc_utils.dbf2df(file).set_index('parcel_id')
        ...
        file = settings['tdm']['main_dir'] + '2035/6_REMM/Ro/volumepoint.dbf'
        s = wfrc_utils.dbf2df(file).set_index('parcel_id')
        svolume = s.RASTERVALU.reindex(parcels.index).fillna(0)
    elif year in range(2046,2099):
        file = settings['tdm']['main_dir'] + '2045/6_REMM/Ro/volumepoint.dbf'
        s = wfrc_utils.dbf2df(file).set_index('parcel_id')
        svolume = s.RASTERVALU.reindex(parcels.index).fillna(0)
    else:
        file = settings['tdm']['main_dir'] + '2011/6_REMM/Ro/volumepoint.dbf'
        s = wfrc_utils.dbf2df(file).set_index('parcel_id')
        svolume = s.RASTERVALU.reindex(parcels.index).fillna(0)
    return svolume
...
def commute_time_20(zones, year, settings):
    if year < 2020:
        file = settings['tdm']['main_dir'] + '2015/6_REMM/Ro/AvgTravelTime.csv'
        z = pd.read_csv(file).set_index('TAZ')
    elif year in range(2020,2028):
        file = settings['tdm']['main_dir'] + '2019/6_REMM/Ro/AvgTravelTime.csv'
        z = pd.read_csv(file).set_index('TAZ')
    elif year in range(2028,2036):
        file = settings['tdm']['main_dir'] + '2027/6_REMM/Ro/AvgTravelTime.csv'
        z = pd.read_csv(file).set_index('TAZ')
    #elif year in range(2031,2035):
    #    file = settings['tdm']['main_dir'] + '2030/' + settings['tdm']['output_volume']
    #    z = wfrc_utils.dbf2df(file).set_index('TAZZ')
    elif year in range(2036,2046):
        file = settings['tdm']['main_dir'] + '2035/6_REMM/Ro/AvgTravelTime.csv'
        z = pd.read_csv(file).set_index('TAZ')
    elif year in range(2046,2099):
        file = settings['tdm']['main_dir'] + '2045/6_REMM/Ro/AvgTravelTime.csv'
        z = pd.read_csv(file).set_index('TAZ')
    else:
        file = settings['tdm']['main_dir'] + '2011/6_REMM/Ro/AvgTravelTime.csv'
        z = pd.read_csv(file).set_index('TAZ')
